#!/bin/bash
# Создаем директорию для middleware, если она не существует
mkdir -p /root/escort-project/server/src/middleware

# Создаем файл api-middleware.ts
cat > /root/escort-project/server/src/middleware/api-middleware.ts << 'API_MIDDLEWARE'
