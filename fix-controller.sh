...
echo "🔧 Исправляем profileController.ts"
...
# Добавляем корректные функции для перемещения профилей в конец файла
...
export const moveProfileUp = async (req: Request, res: Response) => {
  try {
    ...
    // Получаем профиль выше текущего (с меньшим порядковым номером)
...
export const moveProfileDown = async (req: Request, res: Response) => {
  try {
    ...
    // Получаем профиль ниже текущего (с большим порядковым номером)
